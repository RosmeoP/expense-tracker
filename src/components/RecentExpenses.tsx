...
const RecentExpenses: React.FC<RecentExpensesProps> = ({
  ...
  <Card className="w-full h-[355px] border border-gray-200 dark:border-gray-700 shadow-lg bg-white dark:bg-gray-800">
    <CardHeader className="pb-0 bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/30 dark:to-gray-800 rounded-t-xl shadow-sm">
      <CardTitle className="text-base font-semibold flex items-center gap-2 text-blue-900 dark:text-blue-100">
        <HiOutlineReceiptRefund className="text-blue-500 dark:text-blue-400 text-xl" />
        Recent Expenses
      </CardTitle>
    </CardHeader>
    <CardContent className="pt-2">
      <div
        className="overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-600"
        style={{ maxHeight }}
      >
        {expenses.length === 0 ? (
          <div className="px-4 py-8 text-gray-400 dark:text-gray-500 text-center">No recent expenses.</div>
        ) : (
          <ul className="flex flex-col gap-2">
            {expenses.map((exp) => {
              ...
              return (
                <li
                  key={exp.id}
                  className="flex items-center justify-between bg-white dark:bg-gray-700 rounded-lg px-3 py-2 shadow-sm border border-gray-100 dark:border-gray-600 hover:shadow-md hover:bg-blue-50 dark:hover:bg-gray-600 transition-all duration-150"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-gray-800 dark:text-gray-200 text-[15px] flex items-center gap-2">
                        {exp.description}
                        <span className={`ml-1 px-2 py-0.5 rounded-full text-xs font-semibold ${meta.tagColor}`}>
                          {exp.category}
                        </span>
                      </div>
                      <div className="text-xs text-gray-400 dark:text-gray-500">{exp.date}</div>
                    </div>
                  </div>
                  <div className="font-bold text-blue-700 dark:text-blue-400 text-[15px]">${exp.amount.toFixed(2)}</div>
                </li>
              );
            })}
          </ul>

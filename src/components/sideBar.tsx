  ...
  return (
    <>
      ...
      {!mobileOnly && (
        <TooltipProvider>
          <aside
            className="hidden md:flex fixed left-0 top-0 h-full w-64 bg-white dark:bg-gray-900 text-red-200 flex-col shadow-lg z-30 overflow-y-auto border-r border-gray-200 dark:border-gray-700"
            role="navigation"
            aria-label="Sidebar"
          >
      ...
      {mobileOnly && (
        <AnimatePresence>
          <motion.nav
            ...
            animate="visible"
            exit="exit"
            className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[95vw] max-w-md bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex justify-between gap-x-2 px-4 py-2"
            role="navigation"
...
const SidebarSection = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="mb-2">
    <p className="text-gray-600 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider mb-3 px-2">{title}</p>
